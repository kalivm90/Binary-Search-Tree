...
var _Tree = require("./modules/Tree.js");
var _Helper = require("./modules/Helper.js");
...
const main = (() => {
  ...
  const random = (0, _Helper.randomArray)();
  const set = [1, 7, 4, 23, 8, 9, 4, 3, 5, 7, 9, 67, 6345, 324]; // set array for testing reliability 
  const tree = new _Tree.Tree(random);

  // 2. Confirm that the tree is balanced by calling isBalanced
  const balanced = tree.isBalanced(); // true

  // 3, Print out all elements in level, pre, post, and in order
  (0, _Helper.printTraversal)(tree, 1); // second argument is just to denote what transversal number this script is on

  // 4. Unbalance the tree by adding several numbers > 100
  const insertSeveralNums = [...new Set((0, _Helper.randomArray)(4))].forEach(item => tree.insert(item + 100));
  ...
  (0, _Helper.printTraversal)(tree, 2);
})();